  ...
  async execute(
    ...
    try {
      ...
      for (const vote of payload.votes) {
        ...
        const partyAddress = '0x0000000000000000000000000000000000000000' as const
        ...
        if (castVoteResult.isErr) {
          this.logger.error('Failed to cast vote on blockchain:', castVoteResult.error)
          ...
          this.logger.info(`Successfully cast vote for candidate ${vote.candidate_id} on blockchain`)
        }
      }
